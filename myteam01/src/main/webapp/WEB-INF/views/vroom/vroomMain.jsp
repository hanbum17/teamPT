...
<html>
  <head>
    ...
    <%@ include file="../menu/nav.jsp"%> <%@ include file="../menu/footer.jsp"%>
  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="message">떠나고자 하는 지역을 선택해주세요</div>
    </div>

    <script
      type="text/javascript"
      src="//dapi.kakao.com/v2/maps/sdk.js?appkey=bc42aa044cb0d127af995d28498082d8"
    ></script>
    <script>
      // 카카오 지도 API 로드 후 실행
                    ...
                    if (area.name === "Seoul") {
                      fetch("/json/GU_MAP.json")
                        ...
                        .then((guData) => {
                          if (
                            guData.features &&
                            Array.isArray(guData.features)
                          ) {
                            var guAreas = guData.features.map((feature) => ({
                              name: feature.properties.name,
                              path: feature.geometry.coordinates[0].map(
                                (coord) => {
                                  const lat = coord[1];
                                  const lng = coord[0];
                                  return new kakao.maps.LatLng(lat, lng);
                        ...
                        .catch((error) =>
                          console.error("Error loading GU_MAP.json:", error)
                        );
                    }
              ...
              function displayGuArea(guArea) {
                ...
                kakao.maps.event.addListener(
                  ...
                  function (mouseEvent) {
                    ...
                    customOverlay.setContent(
                      '<div class="area">' + guArea.name + "</div>"
                    );
                    customOverlay.setPosition(mouseEvent.latLng);
                    customOverlay.setMap(map);
                  }
                ...
                kakao.maps.event.addListener(guPolygon, "click", function () {
                  ...
                  window.location.href =
                    "/vroom/restaurant?guName=" + encodeURIComponent(guName);
                });
              }
