...
function selectUserList() {
    $.ajax({
        ...
        success: function(result) {
            ...
            document.querySelector('.user-list-container').innerHTML = th; // 테이블 추가
...
function displayData(data) {
    ...
    // <tbody>에 행 추가
    document.querySelector('.user-table tbody').innerHTML += td;
}
        ...
        success: function(result) {
            ...
            let detailPage = `
                <div class="user-detail-card">
                    ...
                    <div class="buttons">
                        <button id="edit-btn">수정</button>&nbsp
                        <button id="delete-btn">삭제</button>
                    </div>
                </div>`;

            // 사용자 상세 정보를 표시할 div에 추가
            document.querySelector('.user-detail-container').innerHTML = detailPage;
...
function handleDeleteUser(user) {
    if (confirm("계정을 삭제하시겠습니까?")) {
        $.ajax({
            type: "post",
            url: "/admin/manage/userDelete",
            data: { userId: user.userId }, // 사용자 ID 전송
            success: function(response) {
                alert(response);
                // 상세 정보 컨테이너를 빈 공간으로 만들기
                document.querySelector('.user-detail-container').innerHTML = '';
                selectUserList(); // 사용자 목록 새로고침
            },
            error: function() {
...
function enableEditMode(user) {
    ...
    // 버튼 변경
    $(".buttons").html(`
        <button id="confirm-btn">확인</button>&nbsp
            ...
            success: function(response) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        $(".buttons").html(`<button id="edit-btn">수정</button>&nbsp<button id="delete-btn">삭제</button>`);
						selectUserList();
						$("#edit-btn").on("click", function() {
		        	   		enableEditMode(result);
				       	});
